...
| [`doc-to-md`](skills/doc-to-md/) | Convert trusted local documents to Markdown with MarkItDown, optional PDF audit bundles, and optional OCR preprocessing. | Local document conversion, textbook-like PDF audit, scanned-PDF OCR preprocessing, and clean Markdown intermediates before analysis. | Curated public skill. Core runtime is pinned MarkItDown; optional book/OCR runtimes are installed explicitly. Codex/macOS arm64 is supported for core/book/OCR on `macos-arm64-py313`; Intel macOS is supported for core/book on `macos-intel-py312`; unlisted Python minors are candidate/unverified; WSL is candidate; Claude Code needs explicit runtime paths; native Windows PowerShell/CMD is unsupported. |
