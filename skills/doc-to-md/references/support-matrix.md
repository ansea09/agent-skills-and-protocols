...
| Claude Code on macOS | Experimental | Claude Code discovers skills from Claude skill locations such as `~/.claude/skills` or `.claude/skills`, but this runtime wrapper still defaults to `${CODEX_HOME:-$HOME/.codex}` and `~/.local/bin`. Configure `CODEX_HOME`, `DOC_TO_MD_BIN_DIR`, and `DOC_TO_MD_TOOLS_DIR`, or install the runtime in the default `.codex` compatibility location. |
