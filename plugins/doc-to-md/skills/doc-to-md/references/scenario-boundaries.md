...
| Codex on macOS 14 or newer on arm64 with rebuilt local runtimes | This is the maintained execution profile. | Supported install, doctors, and smoke selftests. | `scripts/install.sh`, optionally with `--hash-locked` for the maintained `macos-arm64-py313` profile. |
